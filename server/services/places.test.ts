...
describe("photonToCitySuggestions", () => {
  ...
  it("garde la métropole taguée place=province (type=city) en tête, devant un village", () => {
    ...
    const out = photonToCitySuggestions({
      features: [
        {
          properties: { osm_key: "place", osm_value: "province", type: "city", name: "Tokyo", country: "Japon" },
          geometry: { coordinates: [139.69, 35.69] },
        },
        {
          properties: { osm_key: "place", osm_value: "village", type: "city", name: "Tokio", country: "Ukraine" },
  ...
  it("exclut les POI, gares et aéroports (type ≠ city), dont Haneda tagué place=island", () => {
    const out = photonToCitySuggestions({
      features: [
        { properties: { osm_key: "leisure", osm_value: "stadium", type: "house", name: "Tokyo Dome", country: "Japon" }, geometry: { coordinates: [139.7, 35.7] } },
        { properties: { osm_key: "railway", osm_value: "station", type: "house", name: "Tokyo", country: "Japon" }, geometry: { coordinates: [139.7, 35.6] } },
        // Aéroport de Haneda : tagué place=island mais type=house → écarté.
        { properties: { osm_key: "place", osm_value: "island", type: "house", name: "Aéroport International de Tokyo", country: "Japon" }, geometry: { coordinates: [139.78, 35.55] } },
        { properties: { osm_key: "place", osm_value: "country", type: "country", name: "Japon", country: "Japon" }, geometry: { coordinates: [138, 38] } },
      ],
    });
    expect(out).toEqual([]);
  });
  ...
  it("déduplique par label et ignore les entrées sans nom/coordonnées", () => {
    const out = photonToCitySuggestions({
      features: [
        { properties: { osm_key: "place", osm_value: "town", type: "city", name: "Chamonix-Mont-Blanc", country: "France" }, geometry: { coordinates: [6.86, 45.92] } },
        // même label → dédupliqué
        { properties: { osm_key: "place", osm_value: "town", type: "city", name: "Chamonix-Mont-Blanc", country: "France" }, geometry: { coordinates: [6.87, 45.93] } },
        // sans coordonnées → ignoré
        { properties: { osm_key: "place", osm_value: "city", type: "city", name: "SansCoord", country: "France" } },
        // sans nom → ignoré
        { properties: { osm_key: "place", osm_value: "city", type: "city", country: "France" }, geometry: { coordinates: [1, 2] } },
      ],
    });
  ...
  it("retombe sur le nom seul quand le pays est absent", () => {
    const out = photonToCitySuggestions({
      features: [
        { properties: { osm_key: "place", osm_value: "city", type: "city", name: "Singapour" }, geometry: { coordinates: [103.8, 1.35] } },
      ],
    });
    expect(out).toHaveLength(1);
